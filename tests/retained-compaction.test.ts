...
test("retained instructions are omitted only under an identical instruction identity", () => {
  const full = request(false);
  full.context.systemPrompt = ["Preserve this exact policy", "A second system instruction"];
  const snapshot = structuredClone(full);
  const resume = retainedConversationResumeRequest(full)!;
  expect(resume.context.systemPrompt).toEqual([]);
  expect(full).toEqual(snapshot);
  expect(chatGptConversationKey(structuredClone(full), "provider")).toBe(chatGptConversationKey(full, "provider"));
  for (const systemPrompt of [["changed"], [], [...full.context.systemPrompt].reverse()]) {
    const changed = structuredClone(full);
    changed.context.systemPrompt = systemPrompt;
    expect(chatGptConversationKey(changed, "provider")).not.toBe(chatGptConversationKey(full, "provider"));
  }
  const fresh = structuredClone(full);
  fresh.context.messages = [fresh.context.messages[0]!];
  expect(retainedConversationResumeRequest(fresh)).toBeUndefined();
  expect(fresh.context.systemPrompt).toEqual(full.context.systemPrompt);
});
