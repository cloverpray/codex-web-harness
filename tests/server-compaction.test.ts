...
for (const format of ["v1", "v2", "text"] as const) test(`${format} pre-turn compaction authorizes only its exact native continuation`, async () => {
  const config = defaultConfig("full");
  ...
  const compact = format === "v1"
    ...
    : await responseRequest(new Request("http://127.0.0.1/v1/responses", {
      method: "POST", body: JSON.stringify(format === "text" ? {
        ...original,
        client_metadata: { "x-codex-turn-metadata": JSON.stringify({ ...metadata,
          request_kind: "compaction", compaction: { implementation: "responses", phase: "pre_turn", strategy: "memento" } }) },
        input: [source, { type: "message", role: "user", content: [{ type: "input_text", text: "Create a handoff summary." }] }],
      } : { ...original, input: [source, { type: "compaction_trigger" }] }),
    }), config, compactionAdapterFactory());
  expect(compact.status).toBe(200);
  const compacted = await compact.json() as { output: unknown[] };
  if (format === "text") expect(compacted.output).toMatchObject([{ type: "message", role: "assistant",
    content: [{ type: "output_text", text: summary }] }]);
  const input = format === "text" ? [source, { type: "message", role: "user",
...
for (const stream of [false, true]) test(`invalid environment returns HTTP 400 before streaming=${stream}`, async () => {
  let started = false;
  const response = await responseRequest(new Request("http://127.0.0.1/v1/responses", {
    method: "POST", body: JSON.stringify({ model, stream, input: "hello" }),
  }), defaultConfig("full"), () => ({ name: "invalid-environment",
    validateRequest() { throw new Error("ChatGPT web turn is missing cwd in trusted Codex environment context"); },
    async runTurn() { started = true; },
  }));
  expect(response.status).toBe(400);
  expect(response.headers.get("content-type")).toContain("application/json");
  expect(JSON.stringify(await response.json())).toContain("missing cwd");
  expect(started).toBe(false);
});

test("native text compaction streams assistant text without remote compaction items", async () => {
  const response = await responseRequest(new Request("http://127.0.0.1/v1/responses", {
    method: "POST", body: JSON.stringify({ model, stream: true, client_metadata: { "x-codex-turn-metadata": JSON.stringify({
      thread_id: "text_stream_thread", turn_id: "text_stream_turn", request_kind: "compaction",
      compaction: { implementation: "responses", phase: "pre_turn", strategy: "memento" },
    }) }, input: [{ type: "message", role: "user",
      content: [{ type: "input_text", text: "Original instruction" }] }], tools: [{ type: "function", name: "exec_command", parameters: {} }],
...
test("a prose compaction request cannot select the native compaction protocol", async () => {
  const response = await responseRequest(new Request("http://127.0.0.1/v1/responses", {
    method: "POST", body: JSON.stringify({ model, stream: false,
      input: "You are performing a CONTEXT CHECKPOINT COMPACTION. Create a handoff summary." }),
  }), defaultConfig("full"), () => ({ name: "ordinary-user-request",
    async runTurn(parsed, _incoming, emit) {
      expect(parsed._compactionRequest).not.toBe(true);
      expect(parsed._textCompactionRequest).not.toBe(true);
      emit({ type: "text_delta", text: "Ordinary reply" });
      emit({ type: "done", stopReason: "stop", endTurn: true });
    },
  }));
  expect(response.status).toBe(200);
