...
describe("reversible native Codex route integration", () => {
  test("opt-in profile routing leaves native config unchanged through the launcher lifecycle", () => {
    ...
    config.codexConfigProfile = "web";
    saveConfig(config);
    const profilePath = join(codexHome, "web.config.toml");
    const profile = 'model = "chatgpt-web/pro"\n';
    writeFileSync(profilePath, profile);
    expect(getCodexConfigPath()).toBe(profilePath);
    const journal = installCodexIntegration(config);
    expect(journal.configPath).toBe(profilePath);
    ...
    expect(readFileSync(profilePath, "utf8")).toContain("17842/v1");
    uninstallCodexIntegration();
    expect(readFileSync(profilePath, "utf8")).toBe(profile);
    expect(readFileSync(basePath, "utf8")).toBe(base);
  });

  test("profile names cannot escape Codex home and a target switch cannot adopt the old journal", () => {
    const { codexHome } = fixture();
    const config = nativeConfig("browser-only");
    saveConfig(config);
    installCodexIntegration(config);
    const base = readFileSync(join(codexHome, "config.toml"), "utf8");
    saveConfig({ ...config, codexConfigProfile: "../outside" });
    expect(() => getCodexConfigPath()).toThrow("Invalid codexConfigProfile");
    expect(() => loadConfig()).toThrow("Invalid codexConfigProfile");
    saveConfig({ ...config, codexConfigProfile: "web" });
