...
// Large fixture tokenization needs more than Bun's default 5s on hosted Windows.
}, 20_000);
...
test("confirmed retained prompts omit static transport but retain new context and capability", () => {
  const parsed = request("high");
  const capabilities = { localToolsEnabled: true, solAvailable: true, proAvailable: true };
  const token = "turn_12345678901234567890123456789012";
  const full = compileChatGptWebPrompt(parsed, capabilities, token);
  const resumed = compileChatGptWebPrompt(parsed, capabilities, token, { retainedContext: true });
  expect(full.text).toContain("Preserve the task's original instruction priority");
  expect(resumed.text).not.toContain("Preserve the task's original instruction priority");
  expect(resumed.text).toContain("previously supplied transport and original role-priority instructions");
  for (const value of [token, "preserve-system", "preserve-developer", "perform the task", "<codex_transport_resume>"]) {
    expect(resumed.text).toContain(value);
  }
  expect(resumed.text.length).toBeLessThan(full.text.length - 2000);
  // Callers cannot accidentally shorten manual or compaction contracts with this option.
  expect(compileChatGptWebPrompt(parsed, capabilities, token, { retainedContext: true, manualControl: true }).text)
    .toContain("Preserve the task's original instruction priority");
  const compact = { ...parsed, _compactionRequest: true };
  expect(compileChatGptWebPrompt(compact, capabilities, token, { retainedContext: true }).text)
    .toContain("Preserve the task's original instruction priority");
});
