...
describe("ChatGPT outer-native harness v4", () => {
  ...
  test("an invalid environment ends with a nonretryable error before any browser submission", async () => {
    const socketPath = brokerTestEndpoint(`cgw-invalid-env-${process.pid}-${Date.now()}`);
    const provider: CodexProviderConfig = {
      adapter: "chatgpt-web", baseUrl: "browser://invalid-environment-test",
      chatgptWeb: { brokerSocketPath: socketPath, localToolsEnabled: true, solAvailable: true, proAvailable: true },
    };
    const worker = ChatGptBrowserWorker.forProvider(provider);
    const originalRun = worker.run.bind(worker);
    let starts = 0;
    worker.run = async () => { starts++; return "must not submit"; };
    try {
      const request = rawWireRequest("<environment_context><cwd/></environment_context>");
      const events: AdapterEvent[] = [];
      await createChatGptWebAdapter(provider).runTurn!(request, { headers: new Headers() }, event => events.push(event));
      expect(events.filter(event => event.type === "error")).toEqual([expect.objectContaining({
        status: 409, code: "codex_environment_unavailable", retryable: false,
      })]);
  ...
  test("experimental batch preflights all routes and preserves per-item failures and images", async () => {
    const socketPath = brokerTestEndpoint(`cgw-control-batch-${process.pid}-${Date.now()}`);
    const broker = TurnBroker.forSocket(socketPath);
    const environment = extractChatGptTurnEnvironment(parsed(environmentXml));
    environment.tools = [{ name: "exec_command", description: "Run command", parameters: { type: "object" } }];
    ...
    const transport = new StdioClientTransport({
      ...
      env: { ...Object.fromEntries(Object.entries(process.env).filter((entry): entry is [string, string] => typeof entry[1] === "string")), CODEX_CHATGPT_WEB_EXPERIMENTAL_TOOL_BATCH: "1" },
    });
    const client = new Client({ name: "batch-test", version: "1.0.0" });
    const call = (args: Record<string, unknown>) => client.callTool({ name: "codex_tool_batch", arguments: {
      turn_token: token, ...args,
    } });
    const first = { id: "first", wire_name: "exec_command", arguments: { cmd: "pwd" } };
    try {
      await client.connect(transport);
      for (const wire_name of ["codex_exec", "mcp__codex_apps__codex_native2___codex_tool_call", "codex_tool_batch", "unavailable"]) {
        expect((await call({ independent: true, calls: [first, { id: "bad", wire_name }] })).isError).toBeTrue();
      }
      for (const args of [
        { independent: false, calls: [first] },
        { independent: true, calls: [first, first] },
        { independent: true, calls: Array.from({ length: 9 }, (_, i) => ({ ...first, id: String(i) })) },
      ]) expect((await call(args)).isError).toBeTrue();
      const nextBatch = broker.nextToolBatch(token);
      const pending = call({ independent: true, calls: [first, { ...first, id: "second", arguments: { cmd: "false" } }] });
      const batch = await nextBatch;
      // Rejected batches queued nothing; both valid items share one native response round.
      expect(batch).toHaveLength(2);
      const good = batch.find(item => item.arguments?.cmd === "pwd")!;
      const bad = batch.find(item => item.arguments?.cmd === "false")!;
      broker.completeTool(token, bad.callId, { ...toolResult({ message: "command failed" }), isError: true });
      const image = { type: "image", mimeType: "image/png", data: "AA==" };
      broker.completeTool(token, good.callId, { content: [image], structuredContent: { output: "/workspace" } });
      const response = await pending;
      expect(response.isError).toBeTrue();
      expect(response.content).toContainEqual(image);
      expect(response.structuredContent).toMatchObject({ results: [
        { id: "first", structuredContent: { output: "/workspace" } },
        { id: "second", isError: true },
      ] });
      const next = call({ independent: true, calls: [first] });
      const followup = await broker.nextToolBatch(token);
      expect(followup).toHaveLength(1);
      broker.completeTool(token, followup[0]!.callId, toolResult({ output: "still usable" }));
  ...
  test("serves the complete outer-native bridge contract over MCP stdio", async () => {
    ...
    try {
      ...
      const recursiveBridgeNames = [
        "mcp__codex_apps__codex_native2___codex_apply_patch",
        "mcp__codex_apps__codex_native2_codex_exec",
        "codex_tool_call",
      ];
      const bridgeInventory = await inventoryThroughGateway("codex", false, recursiveBridgeNames);
      expect(bridgeInventory.structuredContent).toMatchObject({ total: 1, tools: [{ wire_name: "exec" }] });
      for (const recursiveName of recursiveBridgeNames) {
        const rejected = await call("codex_tool_call", { turn_token: token, wire_name: recursiveName });
        expect(rejected.isError).toBe(true);
        const pending = call("codex_tool_call", {
          turn_token: token, wire_name: "exec", input: `await tools[${JSON.stringify(recursiveName)}]({});`,
        });
        const [request] = await broker.nextToolBatch(token);
        const calls: GatewayProgramCall[] = [];
        await expect(executeGatewayProgram(request!.input!, recursiveBridgeNames, calls, true))
          .rejects.toThrow("Recursive Codex Native bridge routing");
        expect(calls).toEqual([]);
        broker.completeTool(token, request!.callId, { content: [{ type: "text", text: "recursive routing blocked" }], isError: true });
      ...
      for (const wait of agentWaits) {
        ...
        expect(catalog.tools[0]!.description).toContain("timeout_ms=55000");
        ...
        if (wait.direct) {
          ...
          expect(schema.properties.timeout_ms).toEqual({
            type: "number", enum: [30_000, 55_000], minimum: 30_000, maximum: 55_000,
            description: expect.stringContaining("55000 for an evidence-only teacher"),
          });
        ...
        for (const timeout_ms of [30_000, 55_000]) {
        const actualArgs = { ...wait.args, timeout_ms };
        const pending = call("codex_tool_call", { turn_token: token, wire_name: wait.name, arguments: actualArgs });
        const [request] = await broker.nextToolBatch(token);
        if (wait.direct) {
          expect(request).toMatchObject({ wireName: wait.name, arguments: actualArgs });
        } else {
          const calls: GatewayProgramCall[] = [];
          await executeGatewayProgram(request!.input!, [wait.name], calls);
          expect(calls).toEqual([{ name: wait.name, input: actualArgs }]);
        }
        ...
        for (const timeout_ms of [180_000, 30_000, 55_000]) {
