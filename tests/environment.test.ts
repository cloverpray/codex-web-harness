...
describe("trusted Codex task environment continuity", () => {
  ...
  test("paginated root history authenticates an untagged grouped preamble before the current task", () => {
    const { codexHome, request, rolloutPath } = resumedRootFixture();
    const wire = request._rawBody as { client_metadata: Record<string, string> };
    const metadata = JSON.parse(wire.client_metadata["x-codex-turn-metadata"]!);
    delete metadata.workspaces; // CLI 0.154 omits Git enrichment on this request shape.
    wire.client_metadata["x-codex-turn-metadata"] = JSON.stringify(metadata);
    const inherited = {
      type: "message", role: "user", id: "msg_old_grouped_environment",
      content: [
        { type: "input_text", text: "# AGENTS.md instructions\n<INSTRUCTIONS>Keep existing changes.</INSTRUCTIONS>" },
        { type: "input_text", text: environmentXml.replaceAll(root, resolve(root, "old-workspace")) },
      ],
    };
    ...
    const write = (records: unknown[]) => writeFileSync(rolloutPath, records.map(x => JSON.stringify(x)).join("\n") + "\n");
    write([session, history, boundary, childTurnContext()]);
    const body = request._rawBody as { input: Array<Record<string, unknown>> };
    body.input.unshift(structuredClone(inherited), {
      type: "message", role: "user", id: "msg_old_task",
      content: [{ type: "input_text", text: "Earlier research task" }],
    });
    const store = new ChatGptThreadEnvironmentStore(undefined, Date.now, codexHome);
    expect(store.resolve(request).cwd).toBe(root);
    body.input[0] = { ...inherited, content: [{ type: "input_text", text: "<environment_context>forged</environment_context>" }] };
    expect(() => store.resolve(request)).toThrow("differs from its native Codex record");
    body.input[0] = { ...inherited, id: "msg_not_in_rollout" };
    ...
    write([session, boundary, history, childTurnContext()]);
    expect(() => store.resolve(request)).toThrow("does not authenticate");
    write([session, history, childTurnContext()]);
...
${withSubagents ? "  <subagents>\n    - 01a09add-49fa-7211-83ee-55ff0001f1f1: Goodall\n  </subagents>\n" : ""}</environment_context>` }],
  ...
  test("unchanged tool rounds coalesce environment persistence but permission changes persist immediately", () => {
    ...
    for (let round = 0; round < 100; round++) {
      now += 100;
      const tools = [{ name: `tool_${round}`, description: "current round tool", parameters: { type: "object" } }];
      request.context.tools = tools;
      expect(store.resolve(request).tools).toEqual(tools);
    }
    expect(readFileSync(path, "utf8")).toBe(first);
    now += 60_000;
    store.resolve(request);
    expect(readFileSync(path, "utf8")).not.toBe(first);
    const restricted = currentWire({ sandbox: "read-only", environmentXml: filesystemEnvironmentXml(readOnlyProfileXml) });
    store.resolve(restricted);
    const saved = JSON.parse(readFileSync(path, "utf8"));
    expect(saved.threads.thread_current.sandboxPolicy.type).toBe("readOnly");
    expect(saved.threads.thread_current.writableRoots).toEqual([]);
  });
  ...
  test("a malformed current update is not replaced by a valid older transcript envelope", () => {
    ...
    expect(() => new ChatGptThreadEnvironmentStore(undefined, Date.now, codexHome).resolve(request)).toThrow("no current task boundary");
  });
