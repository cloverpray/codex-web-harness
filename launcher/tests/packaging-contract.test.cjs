...
test("launcher publishes native packages for all supported desktop operating systems", () => {
  ...
  assert.equal(manifest.build.artifactName, "code_web_harness_alpha-${version}-${os}-${arch}.${ext}");
...
test("release installers resolve checksummed native launcher assets", () => {
  ...
  assert.match(windowsInstaller, /code_web_harness_alpha-\$Version-win-\$Arch\.exe/);
...
test("CI packages and smoke-launches on macOS, Windows, and Linux", () => {
  ...
  assert.match(ci, /macos-15, macos-15-intel, ubuntu-24.04, windows-2025/);
  ...
  for (const runner of ["macos-15", "macos-15-intel", "ubuntu-24.04", "windows-2025"]) {
  ...
  assert.match(release, /code_web_harness_alpha\.app/);
