...
test("English and Chinese onboarding preserve executable commands and essential links", () => {
  assert.deepEqual(commandFences(chineseReadme), commandFences(englishReadme));
  for (const target of ["docs/GETTING_STARTED.md", "docs/BUILDING.md", "docs/VALIDATION.md", "docs/CHANGES.md", "extras/research", "LICENSE", "CONTRIBUTING.md", "SECURITY.md"]) {
    assert.ok(linkTargets(englishReadme).includes(target), target);
    assert.ok(linkTargets(chineseReadme).includes(target), target);
  }
  assert.ok(linkTargets(japaneseReadme).includes("README.md"));
  assert.ok(linkTargets(japaneseReadme).includes("README.zh-CN.md"));
});
