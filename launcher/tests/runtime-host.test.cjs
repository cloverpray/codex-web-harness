...
test("profile setup rollback restores the Web profile without overwriting concurrent native edits", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "codex-web-profile-checkpoint-"));
  try {
    const coreHome = path.join(root, "core");
    const codexHome = path.join(root, "codex");
    fs.mkdirSync(coreHome);
    fs.mkdirSync(codexHome);
    const config = { codexConfigProfile: "web" };
    const configPath = path.join(coreHome, "config.json");
    fs.writeFileSync(configPath, JSON.stringify(config));
    const base = path.join(codexHome, "config.toml");
    const profile = path.join(codexHome, "web.config.toml");
    fs.writeFileSync(base, "native before\n");
    fs.writeFileSync(profile, "web before\n");
    const host = new RuntimeHost({
      app: { getPath: () => root },
      logger: { info() {}, warn() {}, error() {} },
      sourceRoot: root,
      codexHome,
      supervisor: { coreHome, configPath },
    });
    const checkpoint = host.captureSetupCheckpoint({ owner: "launcher", config });
    fs.writeFileSync(base, "native edited independently\n");
    fs.writeFileSync(profile, "failed web update\n");
    host.restoreSetupCheckpoint(checkpoint);
    assert.equal(fs.readFileSync(profile, "utf8"), "web before\n");
    assert.equal(fs.readFileSync(base, "utf8"), "native edited independently\n");
    assert.throws(() => host.captureSetupCheckpoint({ config: { codexConfigProfile: "../bad" } }), /Invalid codexConfigProfile/);
  } finally {
