...
const en = {
  ...
  proxyBody: "You can configure this before signing in, even offline. Use the HTTP or Mixed port of your proxy app. Applies to the embedded ChatGPT browser and newly started runtime/Tunnel processes; your external browser keeps its own settings. Local Bridge traffic stays direct. Save, then fully quit and reopen this launcher.",
  ...
  supportBody: "If this project helps you, a GitHub star or sharing it on X helps other developers find it. This is optional; you can open the launcher now.",
  star: "Star on GitHub",
...
const zh: Record<keyof typeof en, string> = {
  ...
  proxyBody: "登录前、离线时也可以配置。填写代理软件的 HTTP 或 Mixed 端口，用于内嵌 ChatGPT 浏览器和新启动的服务、Tunnel 进程；外部浏览器使用自身网络设置，本地 Bridge 通信直连。保存后彻底退出并重新打开本软件生效。",
  proxySave: "保存代理设置",
  ...
  continue: "继续",
  supportTitle: "开始之前",
  supportBody: "如果项目对你有帮助，欢迎在 GitHub 点 Star 或在 X 分享，让更多开发者发现它。这是可选项，可以直接进入软件。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  proxyBody: "ログイン前やオフラインでも設定できます。プロキシアプリの HTTP または Mixed ポートを指定します。内蔵 ChatGPT ブラウザと新しいランタイム・Tunnel プロセスに適用し、外部ブラウザは自身の設定を使用します。ローカル Bridge は直接接続します。保存後、このアプリを完全に終了して再起動してください。",
  proxySave: "プロキシ設定を保存",
  ...
  continue: "続ける",
  supportTitle: "開始する前に",
  supportBody: "プロジェクトが役立ったら、GitHub の Star や X での共有で他の開発者にも紹介してください。任意なので、そのままランチャーを開けます。",
  star: "GitHub で Star",
