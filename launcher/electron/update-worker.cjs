...
function updateMac(job) {
  const sourceExecutable = path.join(job.source, "Contents", "MacOS", "code_web_harness_alpha");
  ...
  requireFile(path.join(next, "Contents", "MacOS", "code_web_harness_alpha"), "Copied macOS launcher");
