...
class RuntimeHost {
  ...
  captureSetupCheckpoint(snapshot) {
    ...
    const profile = snapshot.config?.codexConfigProfile;
    ...
    const paths = new Set([
      ...
      ...(profile === undefined ? [] : [path.join(coreHome, "codex", "profile-models.json")]),
      path.join(this.codexHome, profile === undefined ? "config.toml" : `${profile}.config.toml`),
    ...
    return [...paths].map(filePath => captureRegularFile(filePath, {
      followSymlink: filePath === path.join(this.codexHome,
        profile === undefined ? "config.toml" : `${profile}.config.toml`),
    }));
  }
