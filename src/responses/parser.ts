...
export function parseRequest(body: unknown): CodexParsedRequest {
  ...
  // CLI 0.154 pre-turn/mid-turn memento compaction uses ordinary /responses without
  // a compaction_trigger item. Its canonical metadata selects the summarization operation;
  // the final prose instruction alone must never change the protocol of an ordinary task.
  const clientMetadata = isObj(body) && isObj(body.client_metadata) ? body.client_metadata : undefined;
  ...
  if (typeof rawTurnMetadata === "string") {
    try { turnMetadata = JSON.parse(rawTurnMetadata); } catch { turnMetadata = undefined; }
  }
  const textCompactionRequest = isObj(turnMetadata)
    && turnMetadata.request_kind === "compaction"
    && typeof turnMetadata.thread_id === "string" && turnMetadata.thread_id.length > 0
    && typeof turnMetadata.turn_id === "string" && turnMetadata.turn_id.length > 0
    && isObj(turnMetadata.compaction) && turnMetadata.compaction.implementation === "responses";
  let compactionRequest = textCompactionRequest;
  let remoteCompactionTrigger = false;
  ...
  return {
    ...
    ...(textCompactionRequest && !remoteCompactionTrigger ? { _textCompactionRequest: true } : {}),
