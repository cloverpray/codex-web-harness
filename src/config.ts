...
export interface AppConfig {
  version: 3;
  /** Opt-in Codex CLI profile; leaves the base config's model routing untouched. */
...
function parseConfig(value: unknown, path: string): AppConfig {
  ...
  if (parsed.codexConfigProfile !== undefined
    && (typeof parsed.codexConfigProfile !== "string"
      || !/^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$/.test(parsed.codexConfigProfile))) {
    throw new Error(`Invalid codexConfigProfile in ${path}`);
  }
