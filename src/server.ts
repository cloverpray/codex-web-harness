...
import { augmentNativeModelCatalog, syncProfileModelCatalog } from "./model-catalog";
import {
...
export async function modelsRequest(
  ...
  if (config.codexConfigProfile) {
    try {
      const body = JSON.stringify(syncProfileModelCatalog(config));
      return new Response(body, { headers: {
        "content-type": "application/json",
        "etag": `W/"${createHash("sha256").update(body).digest("base64url")}"`,
      } });
    } catch (error) {
      return formatErrorResponse(502, "invalid_response_error",
        `Web profile model catalog unavailable: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
export async function responseRequest(
  ...
  const compactionItemOutput = compaction && !parsed._textCompactionRequest;
  const rememberCompletedResponse = (response: Record<string, unknown>): void => {
    ...
    const summary = parsed._textCompactionRequest && item?.type === "message" && Array.isArray(item.content)
  ...
  try {
    ...
    // Codex retries streamed response.failed even with retryable=false. An actual HTTP 400
    // is required for deterministic authority failures, before any browser turn can start.
    return formatErrorResponse(400, "invalid_request_error", error instanceof Error ? error.message : String(error));
  }
...
export function startServer(
  ...
  // Codex's native and Web sessions share models_cache.json. Seed a dedicated
  // explicit catalog before accepting turns so native refreshes cannot erase Web metadata.
  if (config.codexConfigProfile) syncProfileModelCatalog(config);
