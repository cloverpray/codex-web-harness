...
import { atomicWriteFile, expandUserPath, getConfigDir, getConfigPath } from "./config";
...
export function getCodexConfigPath(): string {
  // Read only the target selector: setup and recovery also call this before a
  // complete runtime configuration exists. Never infer a new target from a journal.
  const configPath = getConfigPath();
  const profile: unknown = existsSync(configPath)
    ? JSON.parse(readFileSync(configPath, "utf8").replace(/^\uFEFF/, "")).codexConfigProfile
    : undefined;
  if (profile === undefined) return join(getCodexHome(), "config.toml");
  if (typeof profile !== "string" || !/^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$/.test(profile)) {
    throw new Error("Invalid codexConfigProfile; expected a simple Codex profile name");
  }
  return join(getCodexHome(), `${profile}.config.toml`);
}
