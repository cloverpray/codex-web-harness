...
export function getProfileCatalogTemplatePath(): string {
  return join(getConfigDir(), "codex", "profile-catalog-template.json");
}

export function getProfileModelCatalogPath(): string {
  return join(getConfigDir(), "codex", "profile-models.json");
}

export function syncProfileModelCatalog(config: AppConfig): JsonObject {
  const catalog = profileModelCatalog(config);
  const path = getProfileModelCatalogPath();
  const text = `${JSON.stringify(catalog, null, 2)}\n`;
  if (!existsSync(path) || readFileSync(path, "utf8") !== text) atomicWriteFile(path, text);
  return catalog;
}

/** Web-only sessions have no native Bearer token. Use the local Codex catalog
 * as a schema template, then advertise only routes implemented by this bridge. */
export function profileModelCatalog(config: AppConfig): JsonObject {
  const source = JSON.parse(readFileSync(getProfileCatalogTemplatePath(), "utf8"));
  const augmented = augmentNativeModelCatalog(source, config);
  return {
    models: (augmented.models as JsonObject[]).filter(model =>
      typeof model.slug === "string" && model.slug.startsWith(CHATGPT_WEB_MODEL_PREFIX)),
  };
}
