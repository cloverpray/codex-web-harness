...
function submittedTurnFailure(session: ChatGptTurnSession, error: unknown): Error {
  ...
  return new ChatGptWebAdapterError(
    ambiguous
      ...
      : identityFailure
        ? "ChatGPT response identity could not be verified after the page changed. The task was stopped to avoid reading another response; inspect existing tool results before continuing."
...
export function createChatGptWebAdapter(
  ...
  const validatedEnvironments = new WeakMap<CodexParsedRequest, ReturnType<typeof extractChatGptTurnEnvironment>>();
  ...
  return {
    ...
    validateRequest(parsed) {
      const manualRequest = isChatGptWebZeroRiskBackendModel(parsed.modelId);
      if ((!parsed._compactionRequest || manualRequest) && configuredCapabilities.localToolsEnabled) {
        validatedEnvironments.set(parsed, environmentStore.resolve(parsed));
      }
    },
    async runTurn(parsed, incoming, emit) {
      ...
      const runChatGptWebTurn = async (): Promise<void> => {
        ...
        if (mode.localTools) {
          try {
            environment = validatedEnvironments.get(parsed) ?? environmentStore.resolve(parsed);
          } catch (error) {
            const identity = extractChatGptTurnIdentity(parsed);
            console.warn(
              `[chatgpt-web] trusted environment unavailable (thread_id=${identity.threadId ?? "missing"}, turn_id=${identity.turnId ?? "missing"}, previous_response_id=${parsed.previousResponseId ?? "none"}, replay_prefix_items=${parsed._replayPrefixLen ?? 0}, context_messages=${parsed.context.messages.length}, current_envelope=${hasCurrentChatGptEnvironmentContext(parsed)}, root_metadata=${Boolean(extractChatGptRootThreadMetadata(parsed))}, child_metadata=${Boolean(extractChatGptThreadSpawnLineage(parsed))})`,
            );
            // Retrying an identical authority claim cannot repair provenance and must not start
            // another browser turn. The caller can recover in a fresh native turn after checking
            // its existing tools/artifacts; do not misclassify this as a retryable stream failure.
            emit({ type: "error", message: error instanceof Error ? error.message : String(error),
              status: 409, errorType: "invalid_request_error", code: "codex_environment_unavailable", retryable: false });
            return;
          }
        }
        if (parsed._compactionRequest) {
