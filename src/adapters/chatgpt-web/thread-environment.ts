...
export class ChatGptThreadEnvironmentStore {
  ...
  private set(threadId: string, environment: ChatGptTurnEnvironment): void {
    ...
    const now = this.now();
    // A tool round often repeats identical authority. Avoid rewriting the entire persisted map
    // each time, but refresh its TTL periodically and persist permission/cwd changes immediately.
    if (previous && now >= previous.updatedAt && now - previous.updatedAt < 60_000
      && sameAuthority({ ...previous, tools: [] }, environment)) return;
    this.threads.delete(threadId);
    this.threads.set(threadId, authority(environment, now));
