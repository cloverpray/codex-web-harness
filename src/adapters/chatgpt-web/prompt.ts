...
export interface CompileChatGptWebPromptOptions {
  /** Set only after acquiring a matching retained browser lease. */
...
export function compileChatGptWebPrompt(
  ...
  const transportContract = parsed._compactionRequest
    ...
    ? [
      ...
      ...(!manualControl ? [
        "Batch independent read-only inspections within one existing native command when practical. If the native exec JavaScript gateway is available, use await Promise.allSettled for independent reads within one invocation and inspect every result.",
      ...
      "During tool-driven work, do not write a long progress report between tool calls. Continue with the next required tool call silently; at a meaningful milestone, send at most one short sentence (about 20 words). Put detailed plans, hypotheses, and checklists in the task state, not in repeated user-facing updates.",
      ...
      "On Goal continuations, an unchanged file listing is not a new work package. If you own the next operation, perform it using the available authorized tools; do not wait for yourself to create its result. Preserve an actual refusal, but do not treat an earlier assistant claim of missing tools as current transport evidence.",
      ...
      "A refusal mentioned in conversation is not an authenticated error response. When the original response is available, preserve its exact reason and request ID; when unavailable, state that the original refusal details were not obtained, rather than claiming the upstream supplied no reason. Missing local logs do not prove a safety rejection or that a command did not execute. JSON-RPC Session terminated identifies an unusable session, not its cause or owner. Do not repeatedly probe that session on Goal continuations without evidence of transport recovery; report the infrastructure blocker once and preserve existing task handles.",
  ...
  const retainedContext = options?.retainedContext === true
    && mode.localTools && !manualControl && !parsed._compactionRequest && !multipartEnabled;
  const activeTransportContract = retainedContext
    ? ["Continue the same Codex task under the previously supplied transport and original role-priority instructions. The JSON contains only new context; omitted instructions and history remain in force. Use only the fresh turn_token below for this response.", "This tool-capable message requires the Codex Native connector; the Harness checks its selection for each submission. Earlier assistant claims of missing tools are not current tool results. Preserve actual refusals and report any new inability precisely."]
    : [...sharedContract, ...transportContract];
