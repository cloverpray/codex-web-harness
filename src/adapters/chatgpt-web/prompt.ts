...
export function compileChatGptWebPrompt(
  ...
  const activeTransportContract = retainedContext
    ? ["Continue the same Codex task under the previously supplied transport and original role-priority instructions. The JSON contains only new context; omitted instructions and history remain in force. Use only the fresh turn_token below for this response."]
