...
import { isChatGptBridgeToolName } from "./bridge-tool-policy";
...
export class TurnBroker implements TurnBrokerOwner {
  ...
  private async dispatch(request: BrokerRequest, socketSignal?: AbortSignal): Promise<unknown> {
    ...
    if (isChatGptBridgeToolName(wireName)) {
      const result = {
        code: "codex_bridge_recursion",
        message: "Recursive Codex Native bridge routing was rejected before dispatch. Use a non-bridge native tool; the current turn remains active.",
        retryable: false,
        turn_active: true,
      };
