...
import { stopSubmittedChatGptTurn } from "./browser-turn-cleanup";
import {
...
export class ChatGptBrowserWorker {
  ...
  private async selectModelAndEffort(
    ...
    // The visible control and the menu's semantic slider checks below are the
    // readiness gates. An unconditional pause here adds latency to every turn.
    await throwIfChatGptRateLimitDialog(page);
  ...
  private async selectConnector(
    ...
    try {
      ...
      while (attemptBudget.triggerAttempts < MAX_CHATGPT_CONNECTOR_TRIGGER_ATTEMPTS) {
        ...
        // Try the ready, focused editor immediately. If hydration drops the
        // mention, the existing bounded retry retains the conservative pacing.
        if (attemptBudget.triggerAttempts > 1) {
          await withBrowserTurnAbort(settleChatGptUi(), abortSignal);
        }
        await composer.pressSequentially(CHATGPT_CONNECTOR_MENTION_QUERY, {
          delay: attemptBudget.triggerAttempts === 1 ? 0 : 25,
          signal: abortSignal,
          timeout: CHATGPT_CONNECTOR_ACTION_TIMEOUT_MS,
        });
  ...
  private async sendAttachedPrompt(
    ...
    // A short retained prompt can enable the button before Lexical/React finishes replacing it.
    // Require readiness on two successive UI polls before focusing and activating it once.
    // Acceptance evidence remains mandatory; never retry a possibly submitted message here.
    const sendEnableDeadline = Date.now() + CHATGPT_SEND_ENABLE_GRACE_MS;
    let enabledOnPreviousPoll = false;
    for (;;) {
      ...
      if (enabled && enabledOnPreviousPoll) break;
      enabledOnPreviousPoll = enabled;
  ...
  private async runBrowserTurn(
    ...
    try {
      ...
      if (diagnosticPage && submissionMayBeRunning) {
        const cleanup = await stopSubmittedChatGptTurn(diagnosticPage);
        console.warn(`[chatgpt-web] browser turn ${turn.traceId} submitted-turn cleanup status=${cleanup.status} attempts=${cleanup.attempts}`);
      }
      if (diagnosticPage && !diagnosticPage.isClosed()) {
